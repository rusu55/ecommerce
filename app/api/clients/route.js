...
export const POST = async (request) =>{
    const {firstName, lastName, address, email, phone, weddingDate, services, value} = await request.json(); 
        ...
        try{
            ...
            const newClient = await Client.create([{firstName: firstName, lastName: lastName, email: email, address: address, phone: phone, value: value, weddingDate: format(new Date(weddingDate), "MM/dd/yyyy"),  project: newProject[0]._id, services: newService[0]._id }], {session: session});        
